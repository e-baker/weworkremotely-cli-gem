...
class Scraper
  ...
  def scrape_category(category)
    ...
    doc.xpath("//item").each do |list_item|
      ...
      @@results << Job.new(id: id, company: title[0], position: title[1], description: list_item.css('description').text, published_date: list_item.css('pubDate').text, url: list_item.css('guid').text, category: category)
    end
  end
